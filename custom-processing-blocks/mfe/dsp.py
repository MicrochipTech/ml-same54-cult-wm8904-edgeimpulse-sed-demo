...
def generate_features(implementation_version, draw_graphs, raw_data, axes, sampling_freq,
    ...
    for ax in range(0, len(axes)):
        ...
        mfe = speechpy.feature.lmfe(signal, sampling_frequency=fs, implementation_version=implementation_version,
        ...
        mfe_cmvn = mfe #speechpy.processing.cmvnw(mfe, win_size=win_size, variance_normalization=False)

        if np.min(mfe_cmvn) != 0 and np.max(mfe_cmvn) != 0 and (np.max(mfe_cmvn) - np.min(mfe_cmvn)) != 0:
