...
def generate_features(implementation_version, draw_graphs, raw_data, axes, sampling_freq,
    ...
    for ax in range(0, len(axes)):
        ...
        filter = lambda x: np.sqrt(np.hanning(x+1))[1:]
        ...
        power_spectrum = speechpy.processing.log_power_spectrum(frames, fft_length)
        ...
        if draw_graphs:
            ...
            if not show_axes:
                cax = ax.imshow(power_spectrum, interpolation='nearest', origin='lower')
